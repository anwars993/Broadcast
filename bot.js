...
client.user.setGame(`BOT BY Anwar : https://discord.gg/rtZ9YHu ` "https://discord.gg/rtZ9YHu")
        ...
        if(!message.channel.guild) return;
            ...
            if(command == "all") {
                if(!message.member.hasPermission("ADMINISTRATOR")) {
                    return message.channel.send("**https://discord.gg/rtZ9YHu`**");
                }
                    if(!args) {
                        return message.reply("**يجب عليك كتابة كلمة او جملة لإرسال البرودكاست**");
                    }
                        message.channel.send(`**هل أنت متأكد من إرسالك البرودكاست؟\محتوى البرودكاست: \`${args}\`**`).then(m => {
                            m.react("✅")
                            .then(() => m.react("❌"));
...
client.on("message", async message => {
    if(message.content.startsWith(prefix + "help")) {
        let help = new Discord.RichEmbed()
            ...
            برودكاست عادي : ${prefix}---> all
            برودكاست للأونلاين فقط : ${prefix} ---> online
                                    **`);
